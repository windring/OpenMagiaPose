...
class VivitTubeletEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values, interpolate_pos_encoding: bool = False):
        ...
        if not interpolate_pos_encoding and (height != self.image_size or width != self.image_size):
            raise ValueError(
                f"pixel_values.shape: {pixel_values.shape}, self.image_size: {self.image_size}, self.patch_size: {self.patch_size}, imcompatible"
            )
...
"""
class VivitMagiaTubeletEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        assert config.image_size == (70, 70)
        assert config.tubelet_size == (2, 2, 2)
        assert config.num_channels == 4096
        assert config.hidden_size == 768
        assert config.groups == 64

        self.projection = nn.Conv3d(
            config.num_channels, config.hidden_size, kernel_size=config.tubelet_size, stride=config.tubelet_size, groups=64
        )
...
"""
class VivitMagiaEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        # self.cls_token = nn.Parameter(torch.zeros(1, 1, config.hidden_size))
        assert config.num_channels == 4096
        self.skip_tokens_projector = nn.Conv1d(config.num_channels, config.hidden_size, kernel_size=2, stride=2, groups=64)
        self.patch_embeddings = VivitMagiaTubeletEmbeddings(config)
        assert self.patch_embeddings.num_patches == 35*35*16

        self.position_embeddings = nn.Parameter(
            torch.zeros(1, self.patch_embeddings.num_patches + NUM_SKIP_TOKENS, config.hidden_size)
        )
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.patch_size = config.tubelet_size[1:]
        self.config = config
    ...
    def forward(self, pixel_values, interpolate_pos_encoding: bool = False):
        # batch_size, num_frames, num_channels, height, width = pixel_values.shape
        batch_size, num_frames, vit7b_hidden_states, num_tokens = pixel_values.shape
        bfchw_tokens = pixel_values[:, :, :, NUM_SKIP_TOKENS:]
        skip_tokens = bfchw_tokens[:, :, :, :NUM_SKIP_TOKENS] # (batch_size, num_frames, vit7b_hidden_states, NUM_SKIP_TOKENS)
        skip_tokens = skip_tokens.permute(0, 2, 1, 3) # (batch_size, num_frames, NUM_SKIP_TOKENS, vit7b_hidden_states)
        skip_tokens = self.skip_tokens_projector(skip_tokens) # (batch_size, num_frames, NUM_SKIP_TOKENS, 768)
        embeddings = self.patch_embeddings(bfchw_tokens, interpolate_pos_encoding=interpolate_pos_encoding) # (batch_size, num_frames, 35*35*16, 768)

        # cls_tokens = self.cls_token.tile([batch_size, 1, 1])
        # embeddings = torch.cat((cls_tokens, embeddings), dim=1)
        embeddings = torch.cat((skip_tokens, embeddings), dim=1)
...
class VivitMambaBlock(nn.Module):
    """Mamba block for sequence modeling, replacing attention mechanism."""
    ...
    def forward(
        ...
        Args:
            ...
            head_mask: Not used in Mamba, kept for interface compatibility
            output_attentions: If True, returns None as second element (Mamba has no attention weights)
        
        Returns:
            Tuple containing:
                - output: (batch, seq_len, hidden_size)
                - attention_weights: None (or empty tuple if output_attentions=False)
        """
        # Mamba expects input of shape (batch, seq_len, dim)
        output = self.mamba(hidden_states)
        
        # Return format compatible with attention layers
        if output_attentions:
            # Mamba doesn't have attention weights, return None
            return (output, None)
        else:
            return (output,)


class VivitMambaLayer(nn.Module):
    """This corresponds to the EncoderBlock class but uses Mamba instead of attention."""
    ...
    def __init__(self, config):
        ...
        self.mamba = VivitMambaBlock(config)
    ...
    def forward(self, hidden_states, head_mask=None, output_attentions=False):
        # Mamba forward pass (replacing self-attention)
        mamba_outputs = self.mamba(
            self.layernorm_before(hidden_states),
            head_mask,
            output_attentions=output_attentions,
        )
        mamba_output = mamba_outputs[0]
        # add attentions if we output attention weights (will be None for Mamba)
        outputs = mamba_outputs[1:]

        # first residual connection
        hidden_states = mamba_output + hidden_states
...
class VivitHybridLayer(nn.Module):
    """Hybrid layer with both Mamba and Attention paths."""
    
    def __init__(self, config, use_mamba=True, use_attention=True):
        super().__init__()
    ...
    def forward(self, hidden_states, head_mask=None, output_attentions=False):
        ...
        # Both paths
        mamba_output = self.mamba(hidden_states, head_mask, False)[0]
        attn_output = self.attention(hidden_states, head_mask, output_attentions)[0]
        
        # Weighted fusion
        mixed_output = (self.fusion_weight[0] * mamba_output + 
                        self.fusion_weight[1] * attn_output)
            
        # Residual connection
        hidden_states = mixed_output + hidden_states
...
class VivitEncoder(nn.Module):
    def __init__(self, config):
        ...
        if config.use_mamba:
            self.layer = nn.ModuleList([VivitMambaLayer(config) for _ in range(config.num_hidden_layers)])
        if config.use_hybrid:
            self.layer = nn.ModuleList([VivitHybridLayer(config) for _ in range(config.num_hidden_layers)])
